...
let
  ...
  cargoSrc = pkgs.lib.cleanSourceWith {
    ...
    filter =
      ...
      || pkgs.lib.hasSuffix "/scripts/check-core-features.sh" (toString path)
...
in
{
  ...
  core-features = craneLibStable.mkCargoDerivation (
    commonArgs
    // {
      cargoArtifacts = cargoArtifactsStable;
      buildPhaseCargoCommand = "bash scripts/check-core-features.sh";
      doInstallCargoArtifacts = false;
    }
  );
}
