...
impl SupervisorRuntime {
    ...
    fn add_child(&mut self, mut child: crate::supervisor::child::ChildSpec) -> CommandResult<u64> {
        ...
        if child.id().is_empty() {
            return Err(ControlError::Rejected(
                crate::supervisor::error::BuildError::InvalidConfig("child id must not be empty"),
            )
            .into());
        }
        ...
        if let Some(&key) = self.children_by_id.get(&id) {
            let error = if self.children[key].membership == MembershipState::Removing {
                ...
                ControlError::Rejected(crate::supervisor::error::BuildError::DuplicateChildId(id))
            };
    ...
    fn remove_child(
        ...
        let (lineage, policy, active) = {
            ...
            let grace = policy.grace().unwrap_or_default();
    ...
    async fn handle_deadlines(&mut self) -> RuntimeResult<()> {
        ...
        let expired_removals: Vec<_> = self
            ...
            .copied()
            .filter_map(|key| {
                let pending = self.children.get(key)?.pending_removal.as_ref()?;
                let grace = pending.policy.grace()?;
                (!pending.grace_expired && pending.grace_deadline <= now).then_some((key, grace))
            })
            .collect();
        for (key, grace) in expired_removals {
            let id = {
                let entry = &mut self.children[key];
...
mod tests {
    ...
    async fn displaced_incarnation_cannot_republish_stable_views_after_rebind() {
        let supervisor = crate::supervisor::Supervisor::dynamic()
            .build()
        ...
        let mut old_runtime = SupervisorRuntime::new(
            ...
            Some(ParentLink {
                ...
                snapshot_cell: crate::supervisor::snapshot::SnapshotCell::new(
