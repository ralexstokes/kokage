...
fn child_identity(event: &LifecycleEvent) -> TestChildIdentity<'_> {
    try_child_identity(event).expect("child transition carries identity")
}

fn assert_envelope_invariant(event: &LifecycleEvent) {
    assert_eq!(
        try_child_identity(event).is_some(),
...
async fn recursive_paths_follow_nested_supervisor_reincarnation_identity() {
    ...
    let builder = Supervisor::ordered()
        .default_child_restart(RestartPolicy::on_failure().limit(3, Duration::from_secs(1)))
...
async fn group_revivable_nested_watch_stays_open_and_resumes() {
    ...
    let running = Supervisor::ordered()
        ...
        .child_spec(ChildSpec::supervisor("leaf", leaf).restart(RestartPolicy::on_failure()))
        .child(
...
async fn rest_for_one_closes_head_but_defers_tail_terminality() {
    ...
    let running = Supervisor::ordered()
        ...
        .child_spec(
            ChildSpec::supervisor("head", head_supervisor).restart(RestartPolicy::on_failure()),
        )
        .child_spec(
            ChildSpec::supervisor("tail", tail_supervisor).restart(RestartPolicy::on_failure()),
        )
        .build()
        .expect("root supervisor builds")
        .spawn();
...
fn middle_supervisor(
    ...
    Supervisor::ordered()
        .child_spec(ChildSpec::supervisor("leaf", leaf).restart(RestartPolicy::never()))
        .child(
            TaskSpec::new("bomb", move |_| {
