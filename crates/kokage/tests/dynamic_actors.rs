...
use kokage::{
    Actor, ActorContext, ActorRef, ActorResult, CancellationHandle, ControlError, DownReason,
    DrainPolicy, DynamicActorOptions, DynamicTree, GraphBuilder, LiveContext, MailboxMode,
...
async fn timed_out_removal_terminates_the_typed_ref() {
    ...
    assert!(matches!(
        handle.dynamic().expect("dynamic scope").remove_child("dynamic").await,
